...
struct vary_node ** second_pass() {
  ...
  for(i=0;i<lastop;i++){
    ...
    for(fnum=startf;fnum<=endf;fnum++){
      struct vary_node * cur = (struct vary_node *) calloc(1,sizeof(struct vary_node));
      cur->value = startv + (endv - startv) * (fnum - startf) / (endf - startf);
      strcpy(cur->name,kname);
      cur->next = knobs[(int)fnum];
      knobs[(int)fnum] = cur;
    }
  }
  return knobs;
